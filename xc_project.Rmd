...
Shit's not working. FFS. Will need to run on residual instead of adjGSPC_mo
```{r}
model1 <- lm(adjGSPC_mo ~ adjFTSE_mo + adjGDAXI_mo + adjFCHI_mo, data=globalIndices)
model1
```
...
We can observe $p-value < .05$ from the DW Test, implying autocorrelation of the residuals in this model. We will attempt to remedy by adding $AR(1)$.

### Fitting $ARIMA(1,0,0)$ or $AR(1)$  
